...
for((i=0; i<5; i++)); do
    nodes=`expr 20 + $i \* 20`
    # echo -e $length >> "wireddata.csv"

    echo -e "ns.tcl: running with $nodes $base_flows $base_pkts \n"
    ns wired1.tcl $nodes $base_flows $base_pkts  
    echo -e "\nparser.awk: running\n"
...
for((i=0; i<5; i++)); do
    ...
    echo -e "ns.tcl: running with $base_nodes $flows $base_pkts \n"
    ns wired1.tcl $base_nodes $flows $base_pkts  
    echo -e "\nparser.awk: running\n"
    awk -f parse.awk trace.tr
done
...
for((i=0; i<5; i++)); do
    ...
    echo -e "ns.tcl: running with $base_nodes $base_flows $pkts \n"
    ns wired1.tcl $base_nodes $base_flows $pkts  
    echo -e "\nparser.awk: running\n"
    awk -f parse.awk trace.tr
done
...
#     ns wired1.tcl $nodes $base_flows $base_length 
#     echo -e "\nparser.awk: running\n"
#     awk -f parse.awk trace.tr
# done
...
#     ns wired1.tcl $base_nodes $flows $base_length 
#     echo -e "\nparser.awk: running\n"
#     awk -f parse.awk trace.tr
# done
